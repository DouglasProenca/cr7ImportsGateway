package com.sistema.cr7ImportsGateway.filter;
...
import com.sistema.cr7ImportsGateway.repository.IGatewayRequestRepository;
import com.sistema.cr7ImportsGateway.service.ErrorReturnTransfer;
...
public class LoggingGlobalRegister implements GlobalFilter, Ordered {
    ...
    private final ErrorReturnTransfer errorReturnTransfer;
    private final IGatewayRequestRepository gatewayRequestRepository;
        	...
        	public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
        	    if (LOGGABLE_CONTENT_TYPES.contains(String.valueOf(getHeaders().getContentType()).toLowerCase())) {
        	        return join(body).flatMap(db -> {
        	            ...
        	            gatewayRequestRepository.save(GatewayRequest.builder()
        	            ...
        	            if (STATUS_CODE_SUCCESS.contains(String.valueOf(exchange.getResponse().getStatusCode().value()))) {
        	                return getDelegate().writeWith(Mono.just(db));
        	            } else {
        	                byte[] bytes = errorReturnTransfer.process(objectMapper, StandardCharsets.UTF_8.decode(db.asByteBuffer()).toString());
